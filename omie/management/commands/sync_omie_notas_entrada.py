...
from omie.services import sincronizar_notas_entrada
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(f"- Notas Criadas: {resumo['criadas']}")
            self.stdout.write(f"- Notas Atualizadas: {resumo['atualizadas']}")
            self.stdout.write(f"- Erros de Nota: {resumo['erros']}")

            if resumo['erros'] > 0:
                self.stdout.write(self.style.WARNING("Detalhes dos erros encontrados:"))
                for erro in resumo['erros_detalhes']:
                    self.stdout.write(self.style.ERROR(f"  * {erro}"))

        except Exception as e:
            raise CommandError(f"Erro catastrófico durante a sincronização com a Omie: {e}")
