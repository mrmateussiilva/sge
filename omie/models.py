...
class OmieRecebimentoNFe(models.Model):
    STATUS_CHOICES = [
        ('PENDENTE', 'Pendente'),
        ('APROVADA', 'Aprovada'),
        ('IMPORTADA', 'Importada'),
    ...
    id_recebimento_omie = models.BigIntegerField(unique=True, verbose_name="ID Recebimento Omie", help_text="nIdReceb da Omie")
    chave_nfe = models.CharField(max_length=60, unique=True, verbose_name="Chave NF-e", help_text="cChaveNFe da Omie")
    modelo = models.CharField(max_length=5, verbose_name="Modelo", help_text="cModeloNFe. Ex: 55 ou 57")
    numero_nf = models.CharField(max_length=30, blank=True, verbose_name="Número NF")
    serie = models.CharField(max_length=20, blank=True, verbose_name="Série")
    
    etapa = models.CharField(max_length=50, blank=True, verbose_name="Etapa")
    ...
    raw_header = models.JSONField(default=dict, blank=True, verbose_name="JSON Header")
    raw_detalhado = models.JSONField(default=dict, blank=True, verbose_name="JSON Detalhado (Itens)")
    erro = models.TextField(blank=True, default="", verbose_name="Log de Erro")
    ...
    def pode_aprovar(self) -> bool:
        if self.status in ['IMPORTADA', 'APROVADA', 'IGNORADA']:
            return False
        itens = self.itens.all()
        if not itens.exists():
            return False
    ...
    def __str__(self):
        return f"Recebimento NF {self.numero_nf or 'S/N'} - {self.fornecedor_nome} ({self.get_status_display()})"


class OmieRecebimentoItem(models.Model):
    STATUS_CHOICES = [
        ('PENDENTE', 'Pendente'),
        ('VINCULADO', 'Vinculado'),
        ('IGNORADO', 'Ignorado'),
    ...
    recebimento = models.ForeignKey(OmieRecebimentoNFe, on_delete=models.CASCADE, related_name='itens')
    sequencia = models.PositiveIntegerField(default=1)

    id_item_omie = models.BigIntegerField(null=True, blank=True, verbose_name="ID Item Omie")
    id_produto_omie = models.BigIntegerField(null=True, blank=True, verbose_name="ID Produto Omie")
    codigo_produto = models.CharField(max_length=100, blank=True, verbose_name="Cód. Produto")

    descricao = models.TextField(verbose_name="Descrição do Item")
    ncm = models.CharField(max_length=20, blank=True, verbose_name="NCM")
    cfop_nfe = models.CharField(max_length=20, blank=True, verbose_name="CFOP NF-e")
    cfop_entrada = models.CharField(max_length=20, blank=True, verbose_name="CFOP Entrada")

    # Quantidades da Nota
    unidade_entrada = models.CharField(max_length=20, blank=True, verbose_name="Unidade Entrada")
    quantidade_entrada = models.DecimalField(max_digits=14, decimal_places=4, default=0.0000, verbose_name="Qtd. Entrada")
    valor_unitario = models.DecimalField(max_digits=14, decimal_places=4, default=0.0000, verbose_name="Val. Unitário")
    valor_total = models.DecimalField(max_digits=14, decimal_places=2, default=0.00, verbose_name="Val. Total")
    ...
    def quantidade_para_entrada(self):
        if self.quantidade_convertida is not None:
            return self.quantidade_convertida
        return self.quantidade_entrada

    def __str__(self):
        return f"Item {self.sequencia} - {self.descricao[:40]}"
